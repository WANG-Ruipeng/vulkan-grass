...
void Renderer::CreateComputeDescriptorSetLayout() {
    // Describe the binding of the descriptor set layout
    // i.e. what types of information will be stored at each binding
    VkDescriptorSetLayoutBinding grassBladesLayoutBinding = {};
    grassBladesLayoutBinding.binding = 0;
    grassBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    grassBladesLayoutBinding.descriptorCount = 1;
    grassBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    grassBladesLayoutBinding.pImmutableSamplers = nullptr;
    ...
    VkDescriptorSetLayoutBinding objectTransLayoutBinding = {};
    objectTransLayoutBinding.binding = 3; 
    objectTransLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    objectTransLayoutBinding.descriptorCount = 1;
    objectTransLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    objectTransLayoutBinding.pImmutableSamplers = nullptr;

    std::vector<VkDescriptorSetLayoutBinding> bindings = { grassBladesLayoutBinding, culledBladesLayoutBinding, numBladesLayoutBinding, objectTransLayoutBinding };
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // Three blades buffers for compute descriptor
        { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(3 * scene->GetBlades().size()) },

        // The collision pos for compute descriptor
        { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER , 1 },
    };
...
void Renderer::CreateGrassDescriptorSets() {
    // Create descriptor sets which point to the model matrix of each group of grass blades
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        modelBufferInfo.offset = 0;
        modelBufferInfo.range = sizeof(ModelBufferObject); // mat4 model matrix
...
void Renderer::CreateComputeDescriptorSets() {
    // Descriptor sets for the compute pipeline
    ...
    std::vector<VkWriteDescriptorSet> descriptorWrites(computeDescriptorSets.size() * 4);
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        VkDescriptorBufferInfo objectTransBufferInfo = {};
        objectTransBufferInfo.buffer = scene->GetBlades()[i]->GetObjectTransBuffer();
        objectTransBufferInfo.offset = 0;
        objectTransBufferInfo.range = sizeof(ObjectTransData); 
        ...
        descriptorWrites[4 * i + 3].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        descriptorWrites[4 * i + 3].descriptorCount = 1;
        descriptorWrites[4 * i + 3].pBufferInfo = &objectTransBufferInfo;
        descriptorWrites[4 * i + 3].pImageInfo = nullptr;
...
void Renderer::CreateGrassPipeline() {
    ...
    rasterizer.depthClampEnable = VK_FALSE;
    rasterizer.rasterizerDiscardEnable = VK_FALSE;
    rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // VK_POLYGON_MODE_LINE; // uncomment to debug tessellation
    rasterizer.lineWidth = 1.0f;
